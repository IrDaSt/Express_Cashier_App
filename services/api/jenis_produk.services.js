...
const getById = async (id_jenis_produk) => {
    const rows = await mysqlconn.query("Select * from jenis_produk where id_jenis_produk=?", [
        id_jenis_produk,
    ]);
    return rows;
};
...
// Create Jenis Produk
const create = async ({ id_jenis_produk = "JNP0000001", id_produk = '',
...
// update Jenis Produk
const update = async ({ id_jenis_produk, nama_jenis_produk, jumlah_produk, harga_produk }) => {
...
const remove = async (id_jenis_produk) => {
    const result = await mysqlconn.query("delete from jenis_produk where id_jenis_produk=?", [
        id_jenis_produk,
    ]);
    return result;
};
