...
jenis_produk_RouterApi.post(
    ...
    async (req, res, next) => {
        ...
        const { id_produk, nama_jenis_produk, jumlah_produk, harga_produk } =
            req.body
        try {
            let result_insert_jenis_produk
            const last_jenis_produk_data =
                await jenis_produk_ServiceApi.getLastData()
            if (last_jenis_produk_data.length === 0) {
                result_insert_jenis_produk =
                    await jenis_produk_ServiceApi.create({
                        id_produk: id_produk,
...
jenis_produk_RouterApi.delete(
    ...
    async (req, res, next) => {
        const id_jenis_produk = req.params.id
        try {
            const result_delete = await jenis_produk_ServiceApi.remove(
                id_jenis_produk,
            )
...
// Get Data by id_produk
jenis_produk_RouterApi.get("/produk/:id", async (req, res, next) => {
    try {
        const data_jenis_produk =
            await jenis_produk_ServiceApi.getDataByIdproduk(req.params.id)
        if (!data_jenis_produk.length) {
            return responses.InternalServerError(res, {
                message: "Not Found",
            })
        }
        responses.Success(res, data_jenis_produk)
    } catch (error) {
        return responses.InternalServerErrorCatch(res, error)
    }
})
...
jenis_produk_RouterApi.get("/:id", async (req, res, next) => {
    try {
        const jenis_produk = await jenis_produk_ServiceApi.getById(
            req.params.id,
        )
